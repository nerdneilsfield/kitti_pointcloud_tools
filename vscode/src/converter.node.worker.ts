...
function heapRange(module: Awaited<ReturnType<typeof createDecoder>>,
                   pointer: number, size: number): void {
...
void (async () => {
  if (!parentPort)
    throw new Error("converter worker has no parent port");
  try {
    const output = await convert(workerData as ConversionData);
    parentPort.postMessage({ ok: true, bytes: output }, [output]);
  } catch (error) {
