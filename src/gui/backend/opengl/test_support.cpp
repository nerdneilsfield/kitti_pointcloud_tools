...
public:
  ...
  readColor(const ViewportRenderer &renderer) override {
    ...
    result.bytes_per_row = static_cast<std::size_t>(result.extent.width) * 4;
    ...
    for (int top = 0, bottom = result.extent.height - 1; top < bottom;
      ...
      const auto top_begin =
          ...
          static_cast<std::ptrdiff_t>(top) *
              static_cast<std::ptrdiff_t>(result.bytes_per_row);
